...
extern unsigned char ucMIDIProgramChangeValue[MAXTRACK8];
...
bool OpenDoSave(HWND hwnd, bool savenew) {
	...
	if (((org_data.TrackFlag() && strstr(mus_file,".org16") == NULL) || (!org_data.TrackFlag() && strstr(mus_file,".org16") != NULL)) || (savenew || gFileUnsaved)) {
		...
		if (res == MSGEXISFILE) {
			if (MessageBox(hwnd, "Do you want to overwrite?", "There is a file with the same name", MB_YESNO | MB_ICONEXCLAMATION)== IDNO) return false;
		}
	}
...
LONG WINAPI OrgCrashHandler(EXCEPTION_POINTERS* ep) {
	...
	MessageBox(NULL, "A fatal error has occurred. The program will now exit.", "An unexpected error has made OrgMaker crash", MB_OK | MB_ICONERROR);
...
{//main function
	...
	for (int j = 0; j < 128; j++)iKeyPhase[j] = -1;
	iCurrentPhase = 0;
	iPushShift[0] = 0;
	iPushShift[1] = 0;
	int vvv;
	...
	for (char i = 0; i < 8; i++)ucMIDIProgramChangeValue[i] = 255;
...
{
//	char str[64];
	int i, j,res;	// 2014.10.18 Added j
	...
	case WM_COMMAND:
		...
		if(timer_sw==0){ //no while playing
			...
			case ID_AC_WAV:
				...
				break;
			case IDM_EXPORT_MIDI:
			case ID_AC_MIDI:
				if (org_data.TrackFlag()) { MessageBox(hWnd,"MIDI only supports OrgData. The file contains Org16Data.","Exporting Notice",MB_OK | MB_ICONINFORMATION); break; }
				...
				//ResetTitlebarChange();
				break;
				//DialogBox(hInst, "DLGMIDIEXPORT", hwnd, OrgData::ExportMIDIData);
				break;
			case IDM_DCLEN:
...
//		MessageBox(hWnd,mus_file,"",MB_OK);
	...
	case WM_DESTROY://for post-processing
		...
		free(gSelectedTheme);
		free(gSelectedWave);
		AllocMessageStringBuffer(false);
		break;

...
//		MessageBox(hWnd,"グリッド","",MB_OK);
	...
	if(flag & SETALLOC){//Allocated space used by notes
...
BOOL OrgData::SetNote(long x,unsigned char y, int DragMode)
{
	...
	if(info.tdata[track].note_list == NULL){
		...
		{
			if (OrgFlagsX[i] == x) note->flag = NULL;
...
//Literally just how you set the volume.
...
//Used for one of Rxo's features.
BOOL OrgData::SetVolume2(long x,unsigned char y,long fade)
{
	...
	if(p->x > x){
		...
		switch(fade){
		case 0: //直線的減衰
			dv = (double)(static_cast<long long>(p->x) - lastx)/(double)lastlength;
			dv = 200 - dv * 200;
...
char OrgData::SetFlag(long x)
{
	NOTELIST* note;//生成NOTE
	NOTELIST* p;//リストを指すポインター
	long u;
	org_data.GetFlagUsed(&u, NULL);
	OrgFlagsX[u] = x;
	char str[26];
	ltoa(u, str, 10);
	for (char j = 0; j < MaxFlag; j++)
	{
		if (OrgFlagsX[j] == x && OrgFlags[FlagFinder(x,false)][1] != org_data.track) return 2;
	}
	//未使用NOTEを検索
	if ((note = SearchNote(info.tdata[track].note_p)) == NULL)return FALSE;
	//初音符ならリストに登録
...
void OrgData::GetFlagUsed(long *use, long *left)
{
	char lf = 0, uf = 0;
	for (char i = 0; i < MaxFlag; i++)
	{
	...
	if(use != nullptr)*use = uf;
	if(left != nullptr)*left = lf;
}

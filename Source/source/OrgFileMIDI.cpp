...
unsigned char ucMIDIProgramChangeValue[MAXTRACK8];
...
BOOL OrgData::ExportMIDIData(char *strMidiFileName, int iRepeat){
	...
	unsigned char flgNoteOff[MAXTRACK8], flg89;
	...
	unsigned char ucCurrentDrumKey[MAXTRACK8];
	...
	//４分音符の分解能
	strMIDIHeader[12] = (unsigned char)((4 * iDeltaTime / 0x100) & 0x7F);
	strMIDIHeader[13] = (unsigned char)((4 * iDeltaTime) & 0xFF);
	...
	//ltempo = 60 * 1000 / (info.wait * 4 * iDeltaTime);
	//invtempo = 60 * 1000000 / (60 * 1000 / (info.wait * 4 ));
...
*/
	...
	//テンポ
	isdl = setDeltaTime((info.line - 1) * 4 * iDeltaTime , 0, p); p+=isdl;  lenbuf+=isdl;
	*p++=0xFF; lenbuf++;
	*p++=0x51; lenbuf++;
	*p++=0x03; lenbuf++;
	//invtempo = 60 * 1000000 / (60 * 1000 / (info.wait * 4 ));
	invtempo = 1000 * info.wait * 4 ;
	tmpuc = (unsigned char)((invtempo / 0x10000) % 0x0100); *p++ = tmpuc; lenbuf++;
	tmpuc = (unsigned char)((invtempo / 0x100) % 0x0100);   *p++ = tmpuc; lenbuf++;
	tmpuc = (unsigned char)((invtempo / 0x1) % 0x0100);     *p++ = tmpuc; lenbuf++;
	...
	*/
	for(i=0;i<MAXMELODY8 ;i++){
		...
		*p++=(unsigned char)((lPvnd >> 8) & 0xFF);
		...
		LastX = -(info.line * 4 * iDeltaTime - 4 - 4 - 4 - 4); lastVol = 100; iUseCode90 = 0;
		LastPan = -99999;
	...
	unsigned char *bufv;
	long x, FinalNoteX;
	int bufsize = 256 * info.line * 4 * 8 * 2, w, t;
	...
	iUse = 0;
	for(t=0, i=MAXTRACK8; i<MAXDRAM8; t++, i++){
	...
	*p++=0x7F;
	lenbuf += 4;
	LastX = -(info.line * 4 * iDeltaTime - 4 - 4 - 4);
	
	for(i=0;i<MAXTRACK8;i++){
		flgNoteOff[i] = 0;
	...
	for(x = 0 ; x <= FinalNoteX+1 ; x++){
		...
		for(t=0, i=MAXMELODY; i<MAXDRAM8; t++, i++){
			if(flgNoteOff[i-8] > 0){
				*p++ = flg89;
				*p++ = 0x89;
		...
		for(t=0, i=MAXMELODY; i<MAXDRAM8; t++, i++){
			w = t * 256 * info.line * 4 * 2 + x * 2 ;
			if((*(bufv+w)) > 0){
				CurrentX = x * iDeltaTime;
				isdl = setDeltaTime(CurrentX, LastX, p);
				p+=isdl;
				LastX = CurrentX;
				ucCurrentDrumKey[i-8] = Convert_Wave_no_to_MIDI_no[info.tdata[i].wave_no];
				if(ucCurrentDrumKey[i-8] == 99){ //Tomのとき
					ucCurrentDrumKey[i-8] = Tom_DrumMap[0];
					for(j = 1; j< 6; j++){
